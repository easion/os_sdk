...
KdFindPointerDriver (char *name)
{
    ...
    for (ret = kdPointerDrivers; ret; ret = ret->next) {
		...
		fprintf(stderr,"%s gots line %d %s\n",__FUNCTION__,__LINE__,ret->name);
...
KdInitInput (void)
{
    ...
    for (dev = kdConfigKeyboards; dev; dev = dev->next) {
		fprintf(stderr, "before KdParseKeyboard\n");
		       ...
		       unsigned char is_up)
{
	...
	//fprintf(stderr,"%s: dpp type %d, code %c events %d\n",__FUNCTION__, type,scan_code,nEvents);
	//fprintf(stderr,"%s: dpp code = %d,%d,%d\n",__FUNCTION__, scan_code,nEvents,is_up);
        for (i = 0; i < nEvents; i++){
			xEvent           *myeee;
			myeee=kdEvents + i;
            KdQueueEvent(ki->dixdev, kdEvents + i);
			fprintf(stderr,"%s: keyButtonPointer code = %c,%c,%d-%d\n",__FUNCTION__, scan_code,key_code,i,myeee->u.u.type);
		}
    }
    else {
