...
KdMapDevice (CARD32 addr, CARD32 size)
{
	...
	mmap_id = new_vm_region("X11ORG", size,0, &new_addr);
	if (mmap_id<0)
	{
		FatalError ("KdMapDevice: failed to new_vm_region (%s)\n",
		    strerror (errno));
	}

	err=remap_vm_region(mmap_id,addr);
	if (err)
	{
		delete_vm_region(mmap_id);
		FatalError ("KdMapDevice: failed to remap_vm_region (%s)\n",
		    strerror (errno));
		return -1;
	}
	return new_addr;
