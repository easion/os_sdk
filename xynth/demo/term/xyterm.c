...
static char* VtGetName(int master)
{
	char buf[32];
	SysIoCtl(master, 0, buf);

	sprintf(path, "/System/Devices/%s", buf);
	return path;
}

static void GetVirtualTerminal(void)
{
    int fd, master;

	fd=SysOpen("/System/Devices/Consoles/VirtualMaster", _SYS_FILE_READ | _SYS_FILE_WRITE, 0);

	if (fd < 0)
	{
		printf("Could not open virtual terminal master device.\n");
		exit(0);
	}

	SysIoCtl(fd, 0, &master);

	ptydev=VtGetName(master);

	slave=SysOpen(ptydev, _SYS_FILE_READ | _SYS_FILE_WRITE, 0);
...
static void xterm_sequence (s_window_t *window, int fd, int op)
{
	...
	switch (op) {
		case 2:
			buf_ = (unsigned char *) s_malloc(strlen((char *) buf) + strlen("NetbasTerminal - "));
			sprintf((char *) buf_, "NetbasTerminal - %s", buf);
...
void unix_set_nonblock(int fd)
{
  ...
  printf("unix_set_nonblock = %x\n",retcode);
  if (retcode == -1 ||
      fcntl(fd, F_SETFL, retcode | O_NONBLOCK) == -1){
	printf("%s:%d,%s", __FILE__,__LINE__,"set_nonblock");
  }

  retcode = fcntl(fd, F_GETFL, 0);

  printf("unix_set_nonblock2 = %x\n",retcode);

}

void get_shell_io(int *read_fd,int *write_fd)
{
	int fd_stdout[2];    /* file descriptors of streams */
	int fd_stdin[2];    /* file descriptors of streams */
	struct stat statb;
	int err;
   ...
   if (fork()==0) {
	   //printf("term:child close %d %d\n",fd_stdin[0],fd_stdout[0]);
	   //printf("term:child dup %d %d\n",fd_stdin[1],fd_stdout[1]);
	   /* child, write data to pipe */
		...
		dup2(fd_stdin[0], STDIN_FILENO);	// 复制管道的子进程端到标准输入
		dup2(fd_stdout[1], STDOUT_FILENO);	// 复制管道的子进程端到标准输出
		dup2(fd_stdout[1], STDERR_FILENO);	// 复制管道的子进程端到标准输出

		close(fd_stdin[0]);
		close(fd_stdout[1]);

		setenv("ENV","/etc/ash.rc",1);

		err = stat("/bin/bash",&statb);
	   ...
	   printf("term:father 111\n");
	   
       //close(fd_stdout[1]);   /* close read end */

	   *read_fd = fd_stdout[0]; //"rb"
	   *write_fd = fd_stdin[1]; //"wb"

	   unix_set_nonblock(fd_stdout[0]);

	   close(fd_stdout[1]);
	   close(fd_stdin[0]);
 	   //printf( "father process:read_fd=%d,write_fd=%d\n",read_fd,write_fd);
...
static void term (s_window_t *window)
{
	...
	//get_tty();
	get_shell_io(&slave,&cmd_fd);
	//get_shell_io(&cmd_fd,&slave);
...
//	chdir("~");
	...
	//SysCreateProcess("/bin/sh", fds, args);
...
int main (int argc, char *argv[])
{
	...
	s_window_set_title(window, "Terminal");
	s_window_set_coor(window, 0, 0, 0, FONTW * SCREENW, FONTH * SCREENH);
