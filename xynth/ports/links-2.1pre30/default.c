...
unsigned char *get_home(int *n)
{
	...
	if (!home) {
		...
		if (!home) {
			...
			printf("home error path_to_exe = %s\n",path_to_exe);
			return NULL;
		}
...
#endif
		if (config_dir){ printf("got line %d\n",__LINE__); goto failed;
}
 printf("got line %d\n",__LINE__);
		goto first_failed;
	}
...
#ifdef GRDRV_X
unsigned char ggr_drv[MAX_STR_LEN] = "x";
#else
unsigned char ggr_drv[MAX_STR_LEN] = "netbasgi";
#endif
