...
#define BOCHS_WIDTH	800
#define BOCHS_HEIGHT 600
#define BOCHS_BPP	32
...
int s_video_bochs_server_init (s_server_conf_t *cfg)
{
	...
	server->window->surface->linear_buf=server->window->surface->vbuf=(unsigned char*)SysMemoryMap(0, server->window->surface->linear_mem_size, 7, memFd, BOCHS_BASE);
