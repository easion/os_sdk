...
//		s_free(server);
	...
	printf("s_handler_add desktop done\n");

	while (server->window->running > 0) {

		if (s_socket_listen_wait(server->window, -1)) {
			printf("s_socket_listen_wait error,exiting ...\n");
			server->window->running = 0;
			break;
		}
	//printf("s_socket_listen_wait desktop done\n");




		//printf("s_server running ...\n");


	}

	//thread_wait(-1, NULL);
	printf("s_server_uninit ...\n");
	s_server_uninit();
