...
#ifdef __netbas__
	int size = sizeof(char) * window->surface->width * window->surface->height;
	addr = attach_vm_region(window->surface->shm_mid, size,NULL);
#else
	#error no netbas
...
void s_surface_linear (s_window_t *window)
{
	...
	//printf("window->surface->shm_mid = %d window->surface->linear_mem_base%d\n",window->surface->shm_mid,window->surface->linear_mem_base);
	...
	#error no netbas
		...
		close(fd);
		#else
			addr = attach_vm_region(window->surface->linear_mem_base, window->surface->linear_mem_size,NULL);
		if (!addr)
		{
			debugf(DCLI | DFAT, "[%d] attach_vm_region [%d] failed", window->id,window->surface->linear_mem_base);
		}
		#endif
	}
	...
	//printf(" attach_vm_region [%p] succ,write video ram", addr);

	//memset((void*)addr,0xff,800*600); //fixme
}
...
void s_surface_uninit (s_window_t *window)
{
	...
	if ((window->type & WINDOW_MAIN) &&
		...
		if (window->surface->need_expose & SURFACE_NEEDEXPOSE) {
			...
			//fixme
			delete_vm_region(window->surface->shm_mid);
			//remove_vm_region(window->surface->linear_buf, window->surface->linear_mem_size);
			//munmap(window->surface->linear_buf, window->surface->linear_mem_size);
		}
		int size = sizeof(char) * window->surface->width * window->surface->height;
		//remove_vm_region(window->surface->matrix, size);
		delete_vm_region(window->surface->shm_mid);
