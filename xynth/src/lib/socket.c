...
/*	data->min_w = window->min_w;
	data->min_h = window->min_h;
	data->max_w = window->max_w;
	data->max_h = window->max_h;*/
...
int s_socket_listen_wait (s_window_t *window, int timeout)
{
	...
	if (window->running <= 0) {
		printf("s_socket_listen_wait not run\n");
		return -1;
	}
	...
	len = window->pollfds->list->nb_elt;
		...
		polfd[i].fd = pfd->fd;
		//printf("s_socket_listen_wait: add [fd = %d]\n", pfd->fd);
	...
	if(ret<0){
		printf("s_socket_api_poll error = %d\n",ret);
	}
	...
	switch (ret) {
		...
		default :
			for (i = 0; i < len; i++) {
				...
				//printf("%s() got line %d 0x%x\n",__FUNCTION__,__LINE__,polfd[i].revents);
...
err:					if (pfd->pf_err != NULL) {
					...
					if (pfd->pf_in != NULL) {
						//printf("%s() got line %d fd%d %p\n",__FUNCTION__,__LINE__,pfd->fd,pfd->pf_in);
...
int s_socket_init_uds (s_window_t *window)
{
	...
	if (s_socket_api_connect(fd, (s_sockaddr_t *) &server, sizeof(server)) == -1) {
		printf("conn to %s failed\n", server.sun_path);
		goto err1;
	}
	debugf(DCLI, "Connected to server via local socket. (%s)", S_SERVER_SOC_NAME);
	return fd;

err1:	s_socket_api_close(fd);
err0:	return -1;
#else
	//#error no uds support
	return -1;
#endif
}
...
int s_socket_init (s_window_t *window)
{
	...
	if (((fd = s_socket_init_uds(window)) < 0) &&
		...
		printf("s_socket_init() error0\n");
		goto err0;
	}
	if (s_socket_init_wakeup(window)) {
		printf("s_socket_init() error2\n");
		goto err0;
	}
	if (s_pollfd_init(&pfd)) {
		goto err0;
