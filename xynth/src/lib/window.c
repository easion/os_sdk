...
//	window->surface->need_expose = SURFACE_NEEDSTREAM;
	//window->surface->need_expose = SURFACE_NEEDEXPOSE;
	s_surface_attach(window);

	//debugf(DSER,"s_window_new 111111\n");
...
int s_window_init (s_window_t **window)
{
	...
	if (s_surface_init(w))       { printf("go err1");goto err1; }
	if (s_pollfds_init(w))       {printf("go err12"); goto err2; }
	if (s_timers_init(w))        { printf("go err3");goto err3; }
	if (s_socket_init(w))        { printf("go err4(1)");goto err4; }
    if (s_handlers_init(w))      { printf("go err4");goto err4; }
	if (s_childs_init(w))        { printf("go err5");goto err5; }
	if (s_eventq_init(w))        { printf("go err6");goto err6; }
	if (s_event_init(&w->event)) { printf("go err7");goto err7; }
	if (s_gettext_init(w))       { printf("go err8");goto err8; }
...
//	w->min_w = w->min_h = w->max_w = w->max_h = -1;
	...
	*window = w;
        ...
        if (s_socket_request(w, SOC_DATA_DISPLAY)) { printf("s_window_init error\n");goto err9; }
...
void * s_window_loop_event (void *arg)
{
	...
	//printf("s_window_loop_event on %X Line %d  \n",getpid(),__LINE__);
	...
	//printf("s_window_loop_event on %X Line %d mask %x\n",getpid(),__LINE__,event->type & EVENT_MASK);
