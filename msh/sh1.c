...
extern char *curdirname(char *nBuf, int sz);

/*
 * default shell, search rules
 */
char	shellname[] = "/bin/sh";
char	search[] = ":/bin:/user/bin:/system/bin";
...
register char **argv;
{
	...
	if (--argc >= 1) {
		...
		if (iof == filechar && --argc > 0) {
			...
			if (newfile(name = *++argv)){
				printf("newfile error %d\n", argc);
				exit(1);
			}
		}
	}
	//printf("dpp: setdash here\n");
	setdash();
	if (e.iop < iostack) {
		PUSHIO(afile, 0, iof);
	...
	if (name && name[0] == '-') {
		...
		if ((f = open("profile", 0)) >= 0)
			next(remap(f));
		if ((f = open("/system/profile", 0)) >= 0){
			//printf("open profile %d ok\n", f);
			next(remap(f));
		}
	}
	if (talking)
