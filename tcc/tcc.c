...
/* path to find crt1.o, crti.o and crtn.o. Only needed when generating
   executables or dlls */
#define CONFIG_TCC_CRT_PREFIX "/user/lib"
...
#endif
#elif defined(TCC_UCLIBC) || defined(JICAMA)
/* currently incorrect */
...
void *resolve_sym(TCCState *s1, const char *sym, int type)
{
    return NULL;//dlsym(RTLD_DEFAULT, sym);
}
...
#if !defined(WIN32) &&!defined(JICAMA) && !defined(CONFIG_TCCBOOT)
...
int tcc_run(TCCState *s1, int argc, char **argv)
{
    ...
    if (do_debug) {
#if defined(WIN32) || defined(CONFIG_TCCBOOT) || defined(JICAMA)
...
#else
    tcc_add_library_path(s, "/user/local/lib");
    tcc_add_library_path(s, "/user/lib");
...
int tcc_set_output_type(TCCState *s, int output_type)
{
    ...
    if (!s->nostdinc) {
        ...
        tcc_add_sysinclude_path(s, "/user/local/include");
        tcc_add_sysinclude_path(s, "/user/include");
#endif
...
int main(int argc, char **argv)
{
   ...
   if (output_type != TCC_OUTPUT_MEMORY) {
	...
	dpp("tcc_set_output_type ok, nb_files=%d\n", nb_files);
