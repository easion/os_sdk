...
int pthread_cond_broadcast(pthread_cond_t *cond)
{
	...
	do
	{
		...
		thread_info sInfo;
		while( thread_get_info( __current_thread->__thread_id, &sInfo ) == 0 )
		{
			if( sInfo.ti_state != TS_READY )
				break;
		}
		thread_resume( __current_thread->__thread_id );
...
int pthread_cond_destroy(pthread_cond_t *cond)
{
	...
	thread_delete_semaphore( cond->__lock );
...
/* If cond was allocated on the heap it might contain random garbage and look
	...
	cond->__lock = thread_create_semaphore( "pthread_cond_lock",  SEM_WARN_DBL_LOCK,1 );
...
int pthread_cond_signal(pthread_cond_t *cond)
{
	...
	/* The thread might not sleep yet */
	thread_info sInfo;
	while( thread_get_info( __waiting_thread->__thread_id, &sInfo ) == 0 )
	{
		if( sInfo.ti_state != TS_READY )
			break;
	}
	
	thread_resume( __waiting_thread->__thread_id );
...
int pthread_cond_timedwait(pthread_cond_t *cond, pthread_mutex_t *mutex, const struct timespec *abstime)
{
	...
	kthread = thread_create( __pt_timer_thread_entry, arg,  __DEFAULT_STACK_SIZE);
	thread_set_name(kthread, "pthread_cond_timer");
...
void __pt_timer_thread_entry( __pt_timer_args *arg )
{
	...
	/* The thread might not sleep yet */
	thread_info sInfo;
	while( thread_get_info( arg->thread, &sInfo ) == 0 )
	{
		if( sInfo.ti_state != TS_READY )
			break;
	}
	thread_resume( arg->thread );
...
int __pt_do_cond_wait( pthread_cond_t *cond, pthread_mutex_t *mutex )
{
	...
	thread_suspend( __waiting_thread->__thread_id );
