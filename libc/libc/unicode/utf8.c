...
/* On success, returns the number of BYTEs taken by the character.    */
/* On failure, returns -103 (invalid scalar value).            */
...
/* or a -101 if an invalid UTF-8 sequence is found.                */
int utf8_stricmp(const UTF8 *s1, const UTF8 *s2)
{
  ...
  for (;;)
  {
    ...
    {
      if ((Res = fd32_utf8to32(s1, &Ch1)) < 0) return -101;
      s1 += Res;
      if ((Res = fd32_utf8to32(s2, &Ch2)) < 0) return -101;
      s2 += Res;
...
/* a positive number if case has been changed or -101 if an  */
...
int utf8_strupr(UTF8 *Dest, const UTF8 *Source)
{
  ...
  for (;;)
  {
    ...
    {
      if ((Res = fd32_utf8to32(Source, &Ch)) < 0) return -101;
      Source += Res;
