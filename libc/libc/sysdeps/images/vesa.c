...
static u32_t lfb_base=0xf8000000;
static u32_t lfb_bpp=32;
static u32_t lfb_scanline=800*4;
...
int openvesadev(int *x, int *y, int *c)
{
	int vbemode;
	long limit;
	MODE_INFO *vinfo;
	...
	vbemode=vesa_mode_match(vinfo, *x, *y, *c);
	
	if (lfb_bpp!=c)printf("vbemode%x base 0x%x, x=%d y=%d bpp=%d\n", vbemode,
	...
	*x=vinfo->XResolution;
	*y=vinfo->YResolution;
	*c=vinfo->BitsPerPixel;
	
	//需要在4m对齐
	if(create_vm_region(&limit,vinfo->PhysBasePtr, "vesa",
		vinfo->XResolution*vinfo->YResolution*vinfo->BitsPerPixel/8, 0x1000)<0)return 0;
	
	_farsetsel(_dos_ds);
	JICAMA_mouse_setup(*x,*y);
	free(vinfo);
